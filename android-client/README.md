...
远程会话进入后台时会立即取消当前触摸输入，但继续保留信令、PeerConnection 和 DataChannel；
回到前台后沿用仍然可用的连接。连接期间使用带常驻通知的 media playback foreground service，
避免系统在后台冻结 WebRTC 和 WSS 网络线程。WebRTC 握手完成后，信令 WSS 已不再是媒体与控制
通道的依赖，因此 WSS 单独失效只记录诊断，不会中断仍可用的远程会话；ICE、PeerConnection 或
DataChannel 明确失败时才清理会话并返回主界面。
